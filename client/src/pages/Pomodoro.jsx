...
export default function Pomodoro() {
  ...
  const onComplete = useCallback(async () => {
    ...
    if (sid) {
      try { await api.patch(`/pomodoro/${sid}/complete`); } catch {}
    }
  ...
  const handleStart = async () => {
    try {
      ...
      localStorage.setItem("tw_pomo_session", JSON.stringify({
        sessionId: id,
        mode,
        startedAt: Date.now(),
  ...
  const handleSessionClick = (s) => {
    ...
    if (!saved) return;
    try {
      const { sessionId: sid, startedAt, duration, mode: savedMode } = JSON.parse(saved);
      if (sid !== s._id) return;
      const elapsed = Math.floor((Date.now() - startedAt) / 1000);
        ...
        <div className="flex items-center min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-white/40 mt-0.5 hidden sm:block">
              {todayFocus} sessions today · {Math.round(((todayFocus * (settings.pomoDuration || 25)) / 60) * 10) / 10}h focused
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5">
          <div className="space-y-4">
            ...
            <div className="card flex flex-col items-center py-10">
              <div className="flex gap-2 mb-8 flex-wrap justify-center">
                {MODES.map((m) => (
                  <button key={m.key} onClick={() => !running && setMode(m.key)} className={clsx("pill", mode === m.key && "active")}>
                    {m.label}
                  </button>
                ))}
              </div>
              ...
              <div className="relative w-56 h-56 mb-6">
                <svg width="224" height="224" viewBox="0 0 224 224" className="rotate-[-90deg]">
                  <circle cx="112" cy="112" r="95" fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth="10" />
                  <circle cx="112" cy="112" r="95" fill="none" stroke={currentMode.ring} strokeWidth="10"
                    strokeDasharray={C} strokeDashoffset={offset} strokeLinecap="round"
                    style={{ transition: "stroke-dashoffset 0.5s ease, stroke 0.3s" }}
                  />
                </svg>
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className={clsx("text-6xl font-light tracking-[-4px] font-mono", currentMode.colorClass)}>
                    {mins}:{secs}
                  </span>
              ...
              <div className="flex items-center gap-4 mb-6">
                <button onClick={handleReset} className="w-12 h-12 rounded-full bg-surface-100 border border-white/[0.07] flex items-center justify-center hover:bg-surface-200 transition-colors">
                  <Icon name="refresh" size={18} color="rgba(255,255,255,0.4)" />
                </button>
                <button
                ...
                >
                  {running ? <Icon name="pause" size={22} color="white" /> : <Icon name="play" size={22} color="white" />}
                </button>
                <button
                  onClick={() => { if (!running) setMode((p) => (p === "focus" ? "short" : "focus")); }}
                ...
                >
                  <Icon name="chevRight" size={18} color="rgba(255,255,255,0.4)" />
                </button>
              </div>
              ...
              <div className="flex gap-2">
                {Array.from({ length: 4 }, (_, i) => (
                  <div key={i}
                    className={clsx("w-2.5 h-2.5 rounded-full transition-all",
                      i < sessionInCycle ? "opacity-100"
                      : i === sessionInCycle && running ? "opacity-100 animate-pulse-soft"
                      : "bg-surface-200 opacity-40"
                    )}
                    style={i < sessionInCycle || (i === sessionInCycle && running) ? { background: currentMode.ring } : undefined}
                  />
                ))}
              </div>
              <div className="flex flex-col items-center gap-1 mt-2">
                <p className="text-xs text-white/25">Session {sessionInCycle + 1} of 4 · Cycle {currentCycle}</p>
                <div className="flex items-center gap-3">
                  <button onClick={() => setManualSession((p) => p - 1)} className="w-6 h-6 rounded-full bg-surface-100 border border-white/[0.07] flex items-center justify-center hover:bg-surface-200 transition-colors">
                    <Icon name="chevLeft" size={12} color="rgba(255,255,255,0.4)" />
                  </button>
                  <button onClick={() => setManualSession((p) => p + 1)} className="w-6 h-6 rounded-full bg-surface-100 border border-white/[0.07] flex items-center justify-center hover:bg-surface-200 transition-colors">
                    <Icon name="chevRight" size={12} color="rgba(255,255,255,0.4)" />
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="card">
              ...
              {sessions.length === 0 ? (
                <p className="text-sm text-white/25 text-center py-4">No sessions yet — start one!</p>
              ) : (
                sessions.map((s) => (
                  <div key={s._id} onClick={() => handleSessionClick(s)}
                    className={clsx(
                      "flex items-center gap-3 p-3 rounded-xl mb-2 border transition-colors",
                      !s.completed && "cursor-pointer hover:bg-primary-400/[0.08]",
                      s.completed ? "bg-teal-300/[0.04] border-teal-300/10" : "bg-primary-400/[0.04] border-primary-400/10",
                    )}
                  >
                    <div className={clsx("w-2 h-2 rounded-full shrink-0", s.completed ? "bg-teal-300" : "bg-primary-400")} />
                    <div className="flex-1">
                      <p className="text-sm font-medium">
                        {s.type === "focus" ? "Focus Session" : s.type === "short_break" ? "Short Break" : "Long Break"}
                      </p>
                      <p className="text-[11px] text-white/30 font-mono mt-0.5">
                        {new Date(s.startedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })} · {s.duration} min
                      </p>
                    </div>
                    <span className={clsx("badge", s.completed ? "badge-green" : "badge-accent")}>
          ...
          <div className="space-y-4">
            <div className="card">
              ...
              {[
                ...
                <div key={key} className="mb-5">
                  <div className="flex justify-between mb-2">
                    ...
                    <span className="text-sm font-medium font-mono text-white">{localSettings[key]} min</span>
                  </div>
                  <input type="range" min={min} max={max} step={1} value={localSettings[key]}
                    onChange={(e) => handleSettingChange(key, e.target.value)}
                    disabled={running} className="w-full cursor-pointer"
                  />
              ...
              <button className="btn btn-primary w-full mt-2" onClick={saveSettings} disabled={savingSettings || running}>
                {savingSettings ? <><Spinner size={14} /> Saving…</> : "Save Settings"}
              </button>
              {running && <p className="text-xs text-white/25 text-center mt-2">Stop timer to change settings</p>}
            </div>
            ...
            <div className="card">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {[
                  { label: "Sessions", value: todayFocus, color: "stat-accent" },
                  { label: "Focus", value: `${Math.round(((todayFocus * (settings.pomoDuration || 25)) / 60) * 10) / 10}h`, color: "stat-teal" },
                ].map((s) => (
                  <div key={s.label} className={clsx("stat-card", s.color, "p-4")}>
