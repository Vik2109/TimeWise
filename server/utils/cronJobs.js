...
const overdueTaskJob = cron.schedule(
  "30 * * * *",
  async () => {
    ...
    try {
      ...
      logger.info(
        `⚠️  Overdue notifications sent for ${overdueTasks.length} tasks`,
      );
    } catch (err) {
...
const habitReminderJob = cron.schedule(
  "0 20 * * *",
  async () => {
    logger.info("⭐ Cron: running habit reminder job");
    try {
      const today = new Date().toISOString().split("T")[0];
      const habits = await Habit.find({ isActive: true }).populate(
        "user",
        "firstName email notifications",
      );
    ...
    try {
      ...
      for (const user of users) {
        try {
          ...
          const focusMinutes = pomodoros.reduce(
            (s, p) => s + (p.duration || 0),
            0,
          );
          const avgHabitRate =
            habits.length > 0
              ? Math.round(
                  habits.reduce((sum, h) => {
                    const done = h.completions.filter(
                      (c) => c.date >= weekAgoStr && c.status === "done",
                    ).length;
          ...
          logger.error(
            `Weekly report failed for user ${user._id}: ${err.message}`,
          );
        }
      }
...
const cleanupJob = cron.schedule(
  ...
  async () => {
    ...
    try {
      ...
      const pomoResult = await Pomodoro.deleteMany({
        completed: true,
        createdAt: { $lt: ninetyDaysAgo },
      });

      logger.info(
        `🗑️  Cleanup: deleted ${notifResult.deletedCount} notifications, ${pomoResult.deletedCount} pomodoro sessions`,
      );
    } catch (err) {
...
// ── Job 7: Pomodoro Start Notifications — runs every minute ──
const pomodoroNotifJob = cron.schedule(
  "* * * * *",
  async () => {
    try {
      const now = new Date();
      ...
      const sessions = await Pomodoro.find({
        type: "focus",
        completed: false,
        startedAt: { $gte: oneMinAgo, $lte: now },
        notified: { $ne: true },
      }).populate("user", "firstName email notifications");
      ...
      for (const session of sessions) {
        ...
        if (!u.notifications?.pomodoro) continue;

        const taskLabel = session.taskTitle || "your task";
        ...
        await createNotif(
          ...
          `A ${session.duration}-minute focus session for "${taskLabel}" has begun.`,
        );

        if (!isQuietHours(u)) {
          await sendPomoNotif(
            u,
            `🍅 Focus session started — ${session.duration} minutes`,
            `Your ${session.duration}-minute focus session for "${taskLabel}" has begun. Stay focused!`,
          );
        }
      ...
      logger.error(`Pomodoro notification job failed: ${err.message}`);
    }
  },
  { scheduled: false },
);
